...
static rt_err_t ifx_configure(struct rt_serial_device *serial, struct serial_configure *cfg)
{
    ...
    rt_kprintf("[UART_CONFIG] name=%s, tx=%d, rx=%d\n",
               uart->config->name, uart->config->tx_pin, uart->config->rx_pin);

    // Step 1: 初始化 UART 硬件
    if (!init_flag)
    {
        result = cyhal_uart_init(uart->config->uart_obj,
                                 uart->config->tx_pin,
                                 uart->config->rx_pin,
                                 NC, NC, NULL, &uart_cfg);
        ...
        {
            rt_kprintf("[ERROR] cyhal_uart_init failed! code=0x%08X\n", result);
            return -RT_ERROR;
        }
    ...
    if (result != CY_RSLT_SUCCESS)
    {
        rt_kprintf("[ERROR] cyhal_uart_set_baud failed! code=0x%08X\n", result);
        return -RT_ERROR;
    }
...
void rt_hw_uart_init(void)
{
    ...
    for (index = 0; index < obj_num; index++)
    {
        ...
        rt_memset(uart_obj[index].config->uart_obj, 0, sizeof(cyhal_uart_t));  // ✅ 加上这句
    ...
    rt_kprintf("Register uart device: %s, result=%d\n", uart_obj[index].config->name, result);
